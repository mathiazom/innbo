...
- [x] Persistent retry queue for failed image uploads — a failed `ImageSync.uploadFull` now enqueues into a local-only `upload_queue` SQLite table ([upload_queue.dart](../client/lib/items/upload_queue.dart)), retried on app foreground/resume with time-based backoff (1min/5min/30min/2hr/1day) and marked permanently failed after 5 retries; a first-run backfill catches images stuck from before this feature shipped. Per-image pending/failed badges in `item_detail_screen.dart` (tap a failed badge to reset and retry), plus an aggregate pending/failed count synced to `paired_device` (`pending_upload_count`/`failed_upload_count`, `backend/migrations/0005_paired_device_upload_status.sql`) so any device can see another device's stuck uploads on the device overview screen. HEIC support and background sync while the app is fully closed remain separate, deferred items (see below).
